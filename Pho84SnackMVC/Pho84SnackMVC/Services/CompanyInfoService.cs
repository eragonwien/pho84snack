...
namespace Pho84SnackMVC.Services
{
   public class CompanyInfoService
   {
      ...
      public class CompanyService : ICompanyInfoService
      {
         ...
         public CompanyService(Pho84SnackContext context)
         {
         ...
         public int Create(CompanyInfo companyInfo)
         {
            using (var con = context.GetConnection())
            {
               con.Open();
               string cmdStr = "insert into COMPANYINFO(Name, Description, Address, AddressExtra, Zip, City, Phone, Email, Facebook) values(:Name, :Description, :Address, :AddressExtra, :Zip, :City, :Phone, :Email, :Facebook);select LAST_INSERT_ID();";
               using (var cmd = new MySqlCommand(cmdStr, con))
               {
